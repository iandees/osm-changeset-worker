...
export async function queryChangesets(
  ...
  if (filters.bbox) {
    ...
    if (geohashes !== null && geohashes.length > 0 && geohashes.length < 2000) {
      ...
      // We inject geohashes directly into SQL to avoid parameter limit issues
      // This is safe because geohashes are generated by our code (alphanumeric)
      const geohashList = geohashes.map(g => `'${g}'`).join(',');
      query += ` AND id IN (SELECT changeset_id FROM changeset_geohashes WHERE geohash IN (${geohashList}))`;
    }
